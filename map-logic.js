...
document.addEventListener('DOMContentLoaded', () = {
    ...
    const baseLayers = {  使用 const 因為它不會被重新賦值
        'Google 街道圖' L.tileLayer('httpsmt1.google.comvtlyrs=m&x={x}&y={y}&z={z}', {
            attribution 'Google Maps'
        }),
        'Google 衛星圖' L.tileLayer('httpsmt1.google.comvtlyrs=s&x={x}&y={y}&z={z}', {
            attribution 'Google Maps'
        }),
        'Google 地形圖' L.tileLayer('httpsmt1.google.comvtlyrs=p&x={x}&y={y}&z={z}', {
            attribution 'Google Maps'
        }),
        'OpenStreetMap' L.tileLayer('https{s}.tile.openstreetmap.org{z}{x}{y}.png', {
            attribution '&copy; a href=httpswww.openstreetmap.orgcopyrightOpenStreetMapa contributors'
        })
    };
    ...
    const LocateMeControl = L.Control.extend({  定義了 LocateMeControl 這個構造函數
        ...
        onAdd function(map) {
            ...
            button.role = button;
            button.innerHTML = 'span class=material-symbols-outlinedmy_locationspan';  使用 Material Symbols Icon
...
window.loadKmlLayer = async (kmlUrl, layerName) = {
    ...
    try {
        ...
        const kmlDoc = parser.parseFromString(kmlText, 'textxml');
...
window.createNavButton = (latlng, name) = {
    ...
    const googleMapsUrl = `httpswww.google.commapssearchapi=1&query=${latlng.lat},${latlng.lng}`;


    const buttonHtml = `
        div class=nav-button-content onclick=window.open('${googleMapsUrl}', '_blank'); event.stopPropagation();
            img src=httpsi0.wp.comcanadasafetycouncil.orgwp-contentuploads201808offroad.png alt=導航 
        div
    `;
