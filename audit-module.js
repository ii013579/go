...
(function() {
    ...
    async function generateLayerCsvReport(kmlId, kmlLayerName, maxPhotos) {
        ...
        try {
            ...
            const folderName = getCleanLayerName(activeKmlId, kmlLayerName);
            const rootPrefix = typeof STORAGE_ROOT !== 'undefined' ? STORAGE_ROOT : 'kmldata-d22fb/storage';
            const csvStoragePath = `${rootPrefix}/${folderName}/${folderName}_清查總表.csv`;
            ...
            if (typeof window.downloadCsvFallback === 'function') {
                const fallbackName = getCleanLayerName(activeKmlId, kmlLayerName);
                window.downloadCsvFallback(csvContent, `${fallbackName}_清查總表.csv`);
            }
        }
    }
    ...
    window.submitNewCustomPoint = async function(formValues) {
        ...
        try {
            // 💡 取得圖層名稱備援機制
            const safeLayerName = kmlLayerName || ns?.currentKmlLayerName || '';

            let photoUrls = [];
            if (typeof window.uploadPhotosToStorage === 'function') {
                photoUrls = await window.uploadPhotosToStorage(photos, kmlId, trimmedPointKey, safeLayerName);
            } else {
                photoUrls = Array.isArray(photos) ? photos.filter(p => typeof p === 'string') : [];
            }

            // 💡 編輯模式處理：移除舊點位資料與舊 Leaflet 地圖標記
            if (isEditMode && oldPointKey) {
                ...
                // 若編輯時更名，清除舊的 Leaflet 標記
                if (oldPointKey !== trimmedPointKey && ns && ns.map) {
                    const currentKmlGroup = ns.geoJsonLayers?.[kmlId] || ns.kmlLayerCache?.[kmlId];
                    const targetGroup = currentKmlGroup || ns.map;
                    targetGroup.eachLayer && targetGroup.eachLayer(layer => {
                        const lName = layer.feature?.properties?.name || layer.feature?.properties?.title || layer.feature?.properties?.auditPointKey;
                        if (lName === oldPointKey) {
                            targetGroup.removeLayer ? targetGroup.removeLayer(layer) : ns.map.removeLayer(layer);
                        }
                    });
                }
            ...
            const layerFolderName = getCleanLayerName(kmlId, safeLayerName);
    ...
    window.uploadPhotosToStorage = async function(photos, kmlId, pointKey, kmlLayerName) {
        ...
        const uploadPromises = photos.map(async (photoData, index) => {
            ...
            const customStoragePath = `${rootPrefix}/${targetLayerFolder}/${safePointKey}_${photoIndexStr}.jpg`;
    ...
    window.deleteCustomPoint = async function(kmlId, pointKey, kmlLayerName) {
        ...
        try {
            ...
            for (let i = 0; i < maxPhotos; i++) {
                ...
                const photoPath = `${STORAGE_ROOT}/${targetLayerFolder}/${safePointKey}_${photoIndexStr}.jpg`;
                try {
